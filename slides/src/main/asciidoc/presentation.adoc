...
Порядок получения событий сохраняется:
...
* 1.5М событий в секунду (_оценка сверху_)
...
----
Для отладки с локального хоста добавить маппинг в `c:\Windows\System32\drivers\etc\hosts`
[source,text]
----
127.0.0.1 kafka
----
